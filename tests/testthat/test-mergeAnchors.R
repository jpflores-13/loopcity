...
regions5k <- GenomicRanges::GRanges(
    seqnames = "chr1",
    IRanges::IRanges(seq(5e3, 95e3, 5e3), width = 5001)
)

regions10k <- GenomicRanges::GRanges(
    seqnames = "chr1",
...
test_that("anchors merge to correct positions", {
    ...
    inters <- InteractionSet::GInteractions(
        anchor1 = c(2, 1, 3),
        anchor2 = c(5, 7, 7),
        regions = regions5k
    )
    ...
    expectedDups <- InteractionSet::GInteractions(
        anchor1 = rep(2, 3),
        anchor2 = c(5, 7, 7),
        regions = regions5k
    ) |>
    ...
    expectedNoDups <- InteractionSet::GInteractions(
        anchor1 = c(2, 2),
        anchor2 = c(5, 7),
        regions = regions5k
    ) |>
