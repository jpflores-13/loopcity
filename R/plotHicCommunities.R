...
plotHicCommunities <- function(pdfName, communities, hicFile, norm = "SCALE",
    ...
    for (i in seq_along(chroms)) {
        ...
        if (missing(zmax)) {
            ...
            if (nrow(z) > 0) {
                zmax <- as.numeric(
                    ceiling(quantile(z[z[, 3] >= 10, 3])[4])
                ) + 100
            } else {
                zmax <- 0
                rlang::abort(paste0(
                    "`zmax` could not be calculated, ",
        ...
        # hic plot
        hicPlot <- plotgardener::plotHicRectangle(
            data = hicFile,
            chrom = chr,
            chromstart = start,
            chromend = end,
        ...
        windowLoops_i <- GenomicRanges::findOverlaps(
            regions,
            communities
        ) |>
            S4Vectors::to()
        ...
        if (is.null(windowLoops$source)) {
            rlang::warn(glue::glue(
                "Cannot differentiate added vs original ",
        ...
        plotgardener::annoHeatmapLegend(hicPlot,
            x = border - 0.25, y = border,
